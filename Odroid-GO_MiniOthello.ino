...
void decidePlace(int player) {  //ボタンを使ってオセロを設置
  ...
  while (pattern != 2) {
    ...
    #else
    if (M5.JOY_Y.isAxisPressed()==1) {  //仮のオセロを下に移動
    #endif
      if (column != 6) {
    ...
    } else if (M5.JOY_Y.isAxisPressed()==2) { //仮のオセロを上に移動
      if (column != 6) {
        M5.Lcd.fillRect(41 + (column * 40), 1 + (row * 40), 39, 39, 0x5e02); //現地点を緑で塗りつぶす
        pattern = 1;
        if (row == 0) {
    ...
    #else
    } else if (M5.JOY_X.isAxisPressed()==1) { //仮のオセロを右に移動
    #endif
    ...
    #ifdef ARDUINO_ODROID_ESP32
    } else if (M5.JOY_X.isAxisPressed()==2) { //仮のオセロを左に移動
